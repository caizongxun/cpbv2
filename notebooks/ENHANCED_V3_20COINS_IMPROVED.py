# ============================================================================
# CPB v2: ENHANCED VERSION 3 - 20 COINS (IMPROVED DATA COLLECTION)
# Better handling of API limits and failures
# ============================================================================

print('='*90)
print('CPB v2: ENHANCED VERSION 3 - 20 COINS (IMPROVED)')
print('Target: Accuracy > 70%, Better data collection')
print('='*90)
...
# ============================================================================
# IMPROVED DATA COLLECTION WITH RETRY LOGIC
# ============================================================================

class ImprovedBinanceCollector:
    """改進的資料蒐集器，具有更好的重試和速率限制"""
    
    BASE_URL = "https://api.binance.com/api/v3"
    MAX_RETRIES = 3
    RETRY_DELAY = [1, 3, 5]  # 指數退避
    REQUEST_DELAY = 0.5  # 請求間隔（秒）
        ...
        while current_start < end_time and len(all_klines) < limit:
            for retry_attempt in range(self.MAX_RETRIES):
                try:
                    ...
                    time.sleep(self.RETRY_DELAY[min(retry_attempt, 2)])
...
print('\n' + '='*90)
print('[STEP 1] Downloading data with improved collection')
print('='*90)

all_data = {}
collector = ImprovedBinanceCollector()

for i, coin in enumerate(CONFIG['coins'], 1):
    try:
        print(f'\n[{i:2d}/20] {coin}...', end=' ', flush=True)
        df = collector.get_klines(coin, '1h', limit=3000)
        if ImprovedBinanceCollector.validate(df):
            all_data[coin] = df
...
for coin in sorted(all_data.keys()):
    ...
    try:
        ...
        trainer = FastTrainer(model, device=device)
