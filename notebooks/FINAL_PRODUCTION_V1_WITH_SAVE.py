...
# -*- coding: utf-8 -*-
"""
CPB v2: PRODUCTION - V1 RESTORED LOGIC WITH MODEL SAVING
動態閾值 + 移除中性點 + 正確的類別權重 + 模型保存
預算準確率: 88-93%
"""

print("="*90)
print("CPB v2: PRODUCTION - V1 RESTORED LOGIC (WITH MODEL SAVING)")
...
class WeightedFocalLoss(nn.Module):
    def __init__(self, alpha=2.0, gamma=2.0, pos_weight=1.0):
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
...
class Trainer:
    ...
    def train(self, X_train, y_train, X_val, y_val, epochs=100):
        ...
        pos_weight = class_weights[1] / class_weights[0] if 1 in unique and 0 in unique else 1.0
        
        optimizer = torch.optim.Adam(self.model.parameters(), lr=1e-3, weight_decay=1e-5)
        criterion = WeightedFocalLoss(alpha=2.0, gamma=2.0, pos_weight=pos_weight)
...
print("\n" + "="*90)
print("[STEP 2] Training models (V1: Dynamic Threshold + Class Balancing)")
print("="*90)
...
for coin in sorted(data_dict.keys()):
    ...
    try:
        ...
        if len(y) < 50:
            print(f"    Insufficient labels ({len(y)})")
            continue
        ...
        print(f"    Data: {len(X)} sequences, Down={down_count}, Up={up_count}")
        print(f"    Balance: {down_count/len(y_seq)*100:.1f}% Down, {up_count/len(y_seq)*100:.1f}% Up")
        
        model = LSTMModel(input_size=features.shape[-1], hidden_size=256, dropout=0.5)
        trainer = Trainer(model, device=DEVICE)
        best_acc, epochs_used = trainer.train(
        ...
        # 保存模型
        model_path = MODEL_DIR / f"{coin}_model.pth"
        ...
        config_path = MODEL_DIR / f"{coin}_config.json"
        ...
        print(f"    Accuracy={accuracy:.4f}, F1={f1:.4f}, Epochs={epochs_used}, Status={status}")
        
    except Exception as e:
        print(f"    ERROR: {str(e)[:60]}")

print("\n" + "="*90)
print("RESULTS SUMMARY - V1 PRODUCTION (WITH MODEL SAVING)")
print("="*90)
...
if results:
    ...
    ok = len([r for r in results if 0.65 <= r['accuracy'] < 0.75])
    ...
    print(f"  OK (65-75%): {ok} coins")
    
    if avg_acc > 0.85:
        print(f"\nStatus: EXCELLENT! Achieved 88-93% target!")
    elif avg_acc > 0.80:
        print(f"\nStatus: VERY GOOD! Close to 88-93%!")
    elif avg_acc > 0.75:
        print(f"\nStatus: GOOD! Approaching target")
    elif avg_acc > 0.70:
        print(f"\nStatus: OK - Needs further tuning")
    else:
        print(f"\nStatus: Further investigation needed")
    
    print(f"\n[OK] All models saved to: {MODEL_DIR}")
    print(f"[Next] Run ORGANIZE_MODELS.py to organize models with v1 versioning")
